...
def evaluate_generator(model_name, seed, experiment_dir, dataset, use_cuda=True):
    ...
    """
    #experiment_dir = "C:/Users/Michael Melody/Desktop/Conditional-Sig-Wasserstein-GANs-master/Conditional-Sig-Wasserstein-GANs-master/exp"
    ...
    #torch.save(x_real, 'real_path.pt')
    ...
    with torch.no_grad():
        ...
        torch.save(sigs_conditional, 'fake_path.pt')
       # (pd.DataFrame(to_numpy(sigs_conditional))).to_csv('bb.csv')
    ...
    #pd.DataFrame(to_numpy(x_fake[0, :1000])).to_csv('mm.csv')
...
def evaluate_benchmarks(algos, base_dir, datasets, use_cuda=False):
    ...
    for dataset_dir in os.listdir(base_dir):
        ...
        for experiment_dir in os.listdir(dataset_path):
            ...
            df.to_csv('summary.csv', decimal=',', sep=';', float_format='%.5f', index=False)
